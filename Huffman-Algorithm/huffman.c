...
int percorreArvore(FILE *saida, NArvore *raiz, NArvore *aux, unsigned char *buffer, int *ini, int tam){
    int j = (*ini);
...
void descomprimirArquivo(const char *arqEntrada, const char *arqSaida){
    ...
    while(feof(entrada) == 0){
        ...
        int qtdChar = percorreArvore(saida, raiz, auxArvore, buffer, ini, tam);
