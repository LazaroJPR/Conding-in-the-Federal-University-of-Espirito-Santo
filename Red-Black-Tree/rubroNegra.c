...
void insertFixup(arvore *T, NodeRN *z){
    while(z->pai->cor == RUBRO){	   //Como z e rubro, a propriedade 4 foi violada

        if(z->pai == z->pai->pai->esq){
            NodeRN *tio = z->pai->pai->dir;

            if(tio->cor == RUBRO){      //Caso o tio seja rubro, temos uma propriedade violada
				z->pai->cor = NEGRO;    //� necessario a troca de cores entre os n�s de referencia
                tio->cor = NEGRO;
                z->pai->pai->cor = RUBRO;
                z = z->pai->pai;
            }
        ...
        else{                              //De forma analoga, realiza-se de forma oposta para caso o av� esteja a direita
...
void insertRN(arvore *T, Artigo *art){
    ...
    while(root != T->sentinela){                   //Laco de repeticao para encontrar onde inserir o n�, mantendo sempre um ponteiro para seu pai
        pai = root;
...
NodeRN *sucessorRN(arvore *T, NodeRN *suc){            //Sucessor do n� � o filho mais a esquerda da subarvore direita
...
void deleteFixup(arvore *T, NodeRN *arv){
    ...
    while((arv != T->raiz) && (arv->cor == NEGRO)){
        if(arv == arv->pai->esq){       //Caso o arv esteja na esquerda
            ...
            else if((aux->esq->cor == NEGRO) && (aux->dir->cor == NEGRO)){   //Caso o "aux" seja negro e os dois filhos dele sejam negros
        ...
        else{           //De forma analoga, realiza-se de forma oposta para caso o avr esteja a direita
            aux = arv->pai->esq;
...
void deleteRN(arvore *T, int ch){
    ...
    free(busca);
    if(corAuxb == NEGRO) deleteFixup(T,auxa);    //Caso o n� removido seja negro, � importante verificar se nao ocorreu
                                                 //mudan�a na altura negra da arvore
}
